...
public class DSPSkyPanel extends Application
{
    ...
    static final Logger logger = Logger.getLogger(encoder.DSPSkyPanel.class.getName());
    ...
    public void start(Stage primaryStage) throws Exception
    {
        ...
        anchorPane.getChildren().addAll(audioOutBox, sampleRateBox, fSelectedField, baseFrequencyLabel, wsprMessageField, gainSlider);
        ...
        modeGroup.selectedToggleProperty().addListener(new ChangeListener<Toggle>()
        {
            ...
            public void changed(ObservableValue<? extends Toggle> observable, Toggle oldValue, Toggle selectedToggle)
            {
                ...
                // Tone or WSPR
                {
                    ...
                    if (s.equals("Tone"))
                    {
                        dBufferOut = dspUtils.makeBufferOut(dspUtils.f0(120), nrOfSamplesPerSymbol);
                    }
                    else
                    {
                        // dBufferOut = dspUtils.makeBufferOut(dspUtils.test(), nrOfSamplesPerSymbol);
                        dBufferOut = dspUtils.makeBufferOut(WSPRSymbols, nrOfSamplesPerSymbol);
                    }
